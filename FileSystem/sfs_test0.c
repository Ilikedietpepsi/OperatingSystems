...
int main() {
    ...
    char out_data[1024];
    int i = sfs_fwrite(f, my_data, sizeof(my_data)+1);
    sfs_fseek(f, 0);
    int j = sfs_fread(f, out_data, sizeof(out_data)+1);
    printf("%s\n", out_data);
    sfs_fclose(f);
    int p = sfs_fopen("some_name.txt");
    sfs_fread(p, out_data, sizeof(out_data)+1);
    printf("%s\n", out_data);
